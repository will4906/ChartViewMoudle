...
public class DrawEngine extends Colleague{
    ...
    private void drawLineFunction(CanvasTool canvasTool, int start, float radius, List<DataPoint> dataList, float screenMove, float chartHeight, MainLineInfo mainLineInfo){
        ...
        for (int i = start; i < dataList.size(); i ++){
            ...
            //点在屏幕上显示的横坐标
            float cx =  pointX - screenMove;
            float pointHeight = changeUserDataToChartViewData(dataList.get(i).getYData(), chartHeight,LEFT_AXIS);
            if (mainLineInfo.isHasLine() && i != 0){
                canvasTool.drawLine(cx - (mainLineInfo.getHorizontalResolution() + radius * 2), changeUserDataToChartViewData(dataList.get(i - 1).getYData(), chartHeight,LEFT_AXIS),cx,pointHeight,mainLineInfo.getPaint());
            }
            if (mainLineInfo.isHasPoint()){
